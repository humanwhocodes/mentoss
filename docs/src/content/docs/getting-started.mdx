...
1.  **Install Mentoss.** If you're using Mentoss in a server-side runtime, you can install it using the package manager of your choice.
    ...
    <Tabs>
    	<TabItem label="npm">
    		<Fragment>
    			```bash title="Install Dependencies" npm install mentoss -D ```
    		</Fragment>
    	</TabItem>
    	<TabItem label="pnpm">
    		<Fragment>
    			```bash title="Install Dependencies" pnpm add mentoss -D ```
    		</Fragment>
    	</TabItem>
    	<TabItem label="yarn">
    		<Fragment>
    			```bash title="Install Dependencies" yarn add mentoss -D ```
    		</Fragment>
    	</TabItem>
    	<TabItem label="Bun">
    		<Fragment>
    			```bash title="Install Dependencies" bun install mentoss -D ```
    		</Fragment>
    	</TabItem>
    </Tabs>
