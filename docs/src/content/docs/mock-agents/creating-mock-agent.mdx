...
## Use the mock agent as the global dispatcher

undici provides `setGlobalDispatcher()` and `getGlobalDispatcher()` to replace the default dispatcher used for all undici requests and Node.js `fetch()` calls that don't specify a dispatcher explicitly. This lets you mock every outgoing HTTP request in a test suite without having to pass `{ dispatcher: agent }` on every call.
...
describe("My API with a global dispatcher", () => {
	const server = new MockServer("https://api.example.com");
	const agent = new MockAgent({
		servers: [server],
	});

	// save the original dispatcher so it can be restored later
	const originalDispatcher = getGlobalDispatcher();

	before(() => {
		// replace the global dispatcher with the mock agent
	...
	it("should return a 200 status code", async () => {
		...
		// no need to pass a dispatcher – the global one is used automatically
		const { statusCode, body } = await request(
			"https://api.example.com/ping",
		);
...
The same pattern works with Node.js `fetch()` — because Node.js `fetch()` internally uses undici, replacing the global dispatcher also intercepts calls made through `fetch()`:
...
describe("My API with global dispatcher and fetch()", () => {
	const server = new MockServer("https://api.example.com");
	const agent = new MockAgent({
		servers: [server],
	});
	...
	it("should intercept fetch() calls", async () => {
		...
		// plain fetch() – no dispatcher option required
		const response = await fetch("https://api.example.com/ping");
...
<Aside type="tip">
	Always restore the original dispatcher in an `after` hook (or equivalent teardown) so that other tests in your suite are not affected.
</Aside>
