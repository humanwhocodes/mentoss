...
[undici](https://undici.nodejs.org/) is a high-performance HTTP client for Node.js. It's used internally by Node.js for `fetch()` and is also available as a standalone library. If you're testing code that uses undici's `request()`, `stream()`, or `pipeline()` methods, you'll want to use `MockAgent`.
...
With your mock agent created, you can now use it with undici.
...
### What's different:
- `MockAgent` does not support `baseUrl` - undici requests always use absolute URLs
...
- `MockAgent` is specifically designed for use with undici, not the browser `fetch()` API
