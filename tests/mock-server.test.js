...
/* globals FormData, Request, TextEncoder, URLSearchParams, Blob */
...
describe("MockServer", () => {
	...
	describe("receive()", () => {
		...
		describe("ResponseCreator Functions", () => {
			...
			it("should return the response generated by a function", async () => {
				server.get("/test", () => ({
					status: 200,
			...
			it("should pass a Request object to the function", async () => {
				server.get("/test", request => {
					...
					assert.strictEqual(request.url, `${BASE_URL}/test?foo=bar`);
					assert.strictEqual(request.method, "GET");
					assert.strictEqual(request.headers.get("custom"), "header");
	...
	describe("traceReceive()", () => {
		...
		it("should not return a response and should return traces with multiple messages when uncalled routes don't match but a called route does match", async () => {
			...
			// now try again
			const { response: response2, traces } =
				await server.traceReceive(request);
	...
	describe("Body Matching", () => {
		...
		it("should match the request when the body is a text Blob", async () => {
		...
		it("should match the request when the body is an image Blob from a Uint8Array", async () => {
			const imageData = new Uint8Array([137, 80, 78, 71]); // Example PNG header
			const blob = new Blob([imageData], { type: "image/png" });
			const expectedResponse = "Image Created";
			...
			server.post(
				...
				{ status: 201, body: expectedResponse },
			);
		...
		it("should not match the request when a different Blob is used", async () => {
	...
	describe("called()", () => {
		...
		it("should throw an error when a GET route is called with POST", async () => {
			server.get("/test", { status: 200, body: "OK" });
			const request = createRequest({
				method: "POST",
				url: `${BASE_URL}/test`,
			...
			assert.throws(() => {
				server.called(request);
			}, /No route matched for POST https:\/\/example.com\/test/u);
		});
		...
		it("should throw an error when request pattern doesn't match any registered routes", () => {
			...
			}, /No route matched for GET https:\/\/example.com\/nonexistent/u);
		});

		it("should throw an error when request pattern matches a route but the method doesn't match any registered routes", () => {
			server.get("/test", { status: 200, body: "OK" });

			// URL pattern exists but method doesn't match, so should return false
			assert.throws(() => {
				server.called({ method: "DELETE", url: "/test" });
			}, /No route matched for DELETE https:\/\/example.com\/test/u);
		});

		it("should throw an error when no routes are registered", () => {
			assert.throws(() => {
				server.called("/test");
			}, /No routes registered to match against/u);
		});
	});
	...
	describe("traceCalled()", () => {
		...
		it("should return no traces when a registered route was not called", () => {
			server.get("/test", { status: 200, body: "OK" });
			...
			assert.ok(result.traces[0].messages.length > 0);
			assert.ok(result.traces[0].title.includes("GET"));
		});

		it("should return no traces when a registered route (POST) was not called", () => {
