...
describe("MockServer", () => {
	...
	describe("receive()", () => {
		...
		describe("ResponseCreator Functions", () => {
			...
			it("should return the response generated by a function", async () => {
				server.get("/test", () => ({ status: 200, headers: { Custom: "header" }, body: "Hi" }));
	...
	describe("Body Matching", () => {
		...
		it("should match the request when the body is an image Blob from a Uint8Array", async () => {
			...
			assert.strictEqual(response.status, 201);
			assert.strictEqual(response.statusText, "Created");
			assert.strictEqual(await getResponseBody(response), expectedResponse);
		});

		it("should not match the request when a different Blob is used", async () => {
			const differentBlob = new Blob(["Different"], { type: "text/plain" });
